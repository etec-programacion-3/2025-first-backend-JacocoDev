...
def update_book(id):
    ...
    for field in ['title', 'author', 'genre', 'published_date', 'isbn', 'status']:
        if field in json_data:
            setattr(book, field, json_data[field])

    db.session.commit()
    return book_schema.jsonify(book)  # Devuelve el libro actualizado o error si no se encuentra o hay datos inválidos.
